...
class _OrderSummaryScreenState extends State<OrderSummaryScreen> {
  ...
  String get _chartModeLabel {
    switch (_chartMode) {
      case _ChartMode.daily:   return 'Daily';
      case _ChartMode.weekly:  return 'Weekly';
      case _ChartMode.monthly: return 'Monthly';
    }
  }
  ...
  List<_ChartPoint> get _chartPoints {
    ...
    switch (_chartMode) {
      case _ChartMode.monthly: return _buildMonthly(orders);
      case _ChartMode.weekly:  return _buildWeekly(orders);
      case _ChartMode.daily:   return _buildDaily(orders);
    }
  }
  ...
  List<_ChartPoint> _buildDaily(List<OrderData> orders) {
    ...
    return sorted
        .map((e) => (label: DateFormat(fmt).format(e.key), amount: e.value.amount, orders: e.value.orders))
        .toList();
  }
  ...
  List<_ChartPoint> _buildWeekly(List<OrderData> orders) {
    ...
    final sorted = map.entries.toList()..sort((a, b) => a.key.compareTo(b.key));
    return sorted
        .map((e) => (label: DateFormat('M/d').format(e.key), amount: e.value.amount, orders: e.value.orders))
        .toList();
  }
  ...
  List<_ChartPoint> _buildMonthly(List<OrderData> orders) {
    ...
    final sorted = map.entries.toList()..sort((a, b) => a.key.compareTo(b.key));
    return sorted
        .map((e) => (label: DateFormat('MMM yy').format(e.value.date), amount: e.value.amount, orders: e.value.orders))
        .toList();
  }
  ...
  List<({String name, int qty, double spent})> get _topProducts {
    ...
    final sorted = map.entries.toList()..sort((a, b) => b.value.qty.compareTo(a.value.qty));
    return sorted.take(5).map((e) => (name: e.key, qty: e.value.qty, spent: e.value.spent)).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final totalDelivery = filtered.fold<double>(0, (s, o) => s + (o.deliveryCharge?.toDouble() ?? 0));
    final totalHandling = filtered.fold<double>(0, (s, o) => s + (o.handlingCharge?.toDouble() ?? 0));
    final totalVat      = filtered.fold<double>(0, (s, o) => s + (o.totalVatAmount?.toDouble() ?? 0));
    final subtotal      = filtered.fold<double>(0, (s, o) => s + (o.totalAmount?.toDouble() ?? 0));
    ...
    final shipped    = filtered.where((o) => o.status?.toLowerCase() == 'shipped' || o.status?.toLowerCase() == 'shipping').length;
    ...
    final statusItems = [
      ...
      _StatusItem('On the Way', shipped,    _cShipped,    Icons.local_shipping_rounded),
      ...
      body: CustomScrollView(
        slivers: [
          ...
          SliverAppBar(
            ...
            title: Text(
              'Order Analytics',
              style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 18.sp, fontFamily: 'Inter'),
            ),
          ),
              ...
              children: [
                ...
                Container(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        _HeaderStat(label: 'Total Spent', value: 'Rs.${totalSpent.toInt()}', icon: Icons.payments_rounded),
                        _vDivider(),
                        _HeaderStat(label: 'Orders', value: '${filtered.length}', icon: Icons.receipt_long_rounded),
                        _vDivider(),
                        _HeaderStat(label: 'Avg Order', value: 'Rs.${avgOrder.toInt()}', icon: Icons.bar_chart_rounded),
                        _vDivider(),
                        _HeaderStat(label: 'Items', value: '$totalItems', icon: Icons.shopping_bag_rounded),
                      ],
                    ),
                  ),
                ),
                  ...
                  child: ListView(
                    ...
                    children: _DR.values.map((r) {
                      ...
                      return GestureDetector(
                        onTap: () => r == _DR.custom ? _pickCustom() : setState(() => _range = r),
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(color: sel ? AppColor.primary : vc.divider),
                          ),
                          child: Text(
                            r.label,
                            style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.w600, color: sel ? Colors.white : vc.onSurface),
                          ),
                        ),
                      );
                    }).toList(),
                ...
                // ── Summary tiles ──────────────────────────────────
                Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: _SectionHeader(title: 'Summary', vc: vc)),
                ...
                if (totalSaved > 0) ...[
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        gradient: LinearGradient(colors: [_cDelivered.withValues(alpha: 0.08), _cDelivered.withValues(alpha: 0.03)]),
                ...
                if (points.isNotEmpty) ...[
                  Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: _SectionHeader(title: 'Spending Over Time', vc: vc)),
                  SizedBox(height: 12.h),
                  Padding(
                    padding: EdgeInsets.symmetric(horizontal: 16.w),
                ...
                // ── Order status ───────────────────────────────────
                Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: _SectionHeader(title: 'Order Status', vc: vc)),
                SizedBox(height: 12.h),
                Padding(
                  padding: EdgeInsets.symmetric(horizontal: 16.w),
                ...
                if (filtered.isNotEmpty) ...[
                  Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: _SectionHeader(title: 'Spending Breakdown', vc: vc)),
                  SizedBox(height: 12.h),
                  Padding(
                    padding: EdgeInsets.symmetric(horizontal: 16.w),
                ...
                if (filtered.isNotEmpty) ...[
                  Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: _SectionHeader(title: 'Orders by Day', vc: vc)),
                  SizedBox(height: 12.h),
                  Padding(
                    padding: EdgeInsets.symmetric(horizontal: 16.w),
                ...
                if (topProducts.isNotEmpty) ...[
                  Padding(padding: EdgeInsets.symmetric(horizontal: 16.w), child: _SectionHeader(title: 'Top Products', vc: vc)),
                  SizedBox(height: 12.h),
                  Padding(
                    padding: EdgeInsets.symmetric(horizontal: 16.w),
...
// ── Header stat cell ──────────────────────────────────────────────────────────
Widget _vDivider() => Container(width: 1, height: 32, color: Colors.white.withValues(alpha: 0.25));
  ...
  Widget build(BuildContext context) => Expanded(
        child: Column(
          ...
          children: [
            ...
            Text(label, style: TextStyle(fontSize: 9.sp, color: Colors.white.withValues(alpha: 0.70))),
          ],
        ),
      );
}
...
class _SpendingChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final minNonZero = points.map((p) => p.amount).where((a) => a > 0)
        ...
        children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('Rs.${totalSpent.toInt()}',
                          style: TextStyle(fontSize: 20.sp, fontWeight: FontWeight.w800, color: vc.onSurface)),
                      SizedBox(height: 2.h),
                      Text('$periodLabel  •  $orderCount order${orderCount == 1 ? '' : 's'}',
          ...
          if (maxY > 0) ...[
            ...
            Padding(
              ...
              child: Row(
                children: [
                  _ChartStat(label: 'Peak', value: 'Rs.${maxY.toInt()}', color: AppColor.primary),
                  ...
                  _chartStatDivider(vc),
                  _ChartStat(
                    label: mode == _ChartMode.daily ? 'Active Days' : 'Active Periods',
                    value: '$activePts',
                    color: _cDelivered,
                  ),
                ],
...
class _LineChart extends StatelessWidget {
  ...
  const _LineChart({required this.points, required this.maxY, required this.vc});
    ...
    return LineChart(
      LineChartData(
        ...
        gridData: FlGridData(
          ...
          getDrawingHorizontalLine: (_) => FlLine(color: vc.divider, strokeWidth: 0.8),
        ),
        ...
        titlesData: FlTitlesData(
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              reservedSize: 24,
              interval: points.length > 14 ? (points.length / 6).ceilToDouble() : 1,
              getTitlesWidget: (v, _) {
                final i = v.toInt();
                if (i < 0 || i >= points.length) return const SizedBox();
                return Padding(
                  padding: const EdgeInsets.only(top: 4),
                  child: Text(points[i].label, style: TextStyle(fontSize: 8.sp, color: vc.onSurfaceMuted)),
                );
              },
            ),
          ),
        ...
        titlesData: FlTitlesData(
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              showTitles: true,
              reservedSize: 24,
              interval: points.length > 12 ? (points.length / 6).ceilToDouble() : 1,
              getTitlesWidget: (v, _) {
                final i = v.toInt();
                if (i < 0 || i >= points.length) return const SizedBox();
                return Padding(
        ...
        barGroups: List.generate(points.length, (i) {
          ...
          return BarChartGroupData(
            ...
            barRods: [
              BarChartRodData(
                toY: points[i].amount,
                color: active ? AppColor.primary : AppColor.primary.withValues(alpha: 0.12),
                width: barWidth,
...
class _ChartStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
        ...
        children: [
          ...
          Text(label, style: TextStyle(fontSize: 9.sp, color: context.vColors.onSurfaceMuted)),
        ],
      );
}
            ...
            child: Stack(
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    Text('$total',
                        style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w800, color: vc.onSurface)),
                    Text('orders', style: TextStyle(fontSize: 8.sp, color: vc.onSurfaceMuted)),
                  ],
                ),
              ],
            ),
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(s.label,
                                style: TextStyle(fontSize: 11.sp, fontWeight: FontWeight.w600, color: vc.onSurface)),
                          ),
                          Container(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(fontSize: 9.sp, fontWeight: FontWeight.w700, color: s.color),
                            ),
                          ),
                        ],
                      ),
...
class _SpendingBreakdownCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          _BreakdownRow(label: 'Subtotal', value: 'Rs.${subtotal.toInt()}', color: vc.onSurface, vc: vc),
          if (delivery > 0) _BreakdownRow(label: 'Delivery Charges', value: '+ Rs.${delivery.toInt()}', color: _cPending, vc: vc),
          if (handling > 0) _BreakdownRow(label: 'Handling Charges', value: '+ Rs.${handling.toInt()}', color: _cPending, vc: vc),
          if (vat > 0) _BreakdownRow(label: 'VAT', value: '+ Rs.${vat.toInt()}', color: vc.onSurfaceMuted, vc: vc),
          if (saved > 0) _BreakdownRow(label: 'Discounts & Coupons', value: '− Rs.${saved.toInt()}', color: _cDelivered, vc: vc),
          Divider(color: vc.divider, height: 20.h),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text('Total Paid', style: TextStyle(fontSize: 13.sp, fontWeight: FontWeight.w700, color: vc.onSurface)),
              Text('Rs.${total.toInt()}', style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.w800, color: AppColor.primary)),
            ],
          ),
        ],
      ),
...
class _BreakdownRow extends StatelessWidget {
  ...
  final Color color;
  final VhandarColors vc;
  const _BreakdownRow({required this.label, required this.value, required this.color, required this.vc});
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(label, style: TextStyle(fontSize: 12.sp, color: vc.onSurfaceMuted)),
            Text(value, style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.w600, color: color)),
          ],
        ),
      );
}
        ...
        children: [
          if (busiestIdx >= 0) ...[
            Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.star_rounded, color: AppColor.primary, size: 13.sp),
                  SizedBox(width: 5.w),
                  Text(
                    'You order most on ${_days[busiestIdx]}s',
                    style: TextStyle(fontSize: 11.sp, fontWeight: FontWeight.w600, color: AppColor.primary),
                  ),
                ],
              ),
            ),
          ...
          Row(
            ...
            children: List.generate(7, (i) {
              ...
              final barColor = isMax ? AppColor.primary : AppColor.primary.withValues(alpha: 0.35);

              return Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  if (count > 0)
                    Text('$count', style: TextStyle(fontSize: 9.sp, fontWeight: FontWeight.w700, color: barColor)),
                  SizedBox(height: 3.h),
                  AnimatedContainer(
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(p.name,
                          style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.w600, color: vc.onSurface),
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis),
                    ),
                    SizedBox(width: 8.w),
                    ...
                    Text('Rs.${p.spent.toInt()}',
                        style: TextStyle(fontSize: 11.sp, fontWeight: FontWeight.w700, color: vc.onSurface)),
                  ],
                ),
                SizedBox(height: 6.h),
                ClipRRect(
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
        children: [
          ...
          SizedBox(width: 8.w),
          Text(title,
              style: TextStyle(fontSize: 14.sp, fontWeight: FontWeight.w700, color: vc.onSurface)),
        ],
      );
}
        ...
        children: [
          Align(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: color.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(7.r)),
          ...
          Text(value,
              ...
              overflow: TextOverflow.ellipsis),
          SizedBox(height: 1.h),
          Text(label, style: TextStyle(fontSize: 11.sp, color: vc.onSurface, fontWeight: FontWeight.w600)),
