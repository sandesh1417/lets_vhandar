import 'package:flutter/material.dart';

class AppColor {
  // static Color bg = const Color(0xFF121212).withOpacity(1);
  // static Color btn = const Color(0xFF242A38).withOpacity(1);
  static Color btn = white02;
  // static Color bg = const Color(0xFF1A1E29).withOpacity(1);
  static Color bg08 = const Color(0xFF1A1E29).withOpacity(0.8);
  static Color bg06 = const Color(0xFF1A1E29).withOpacity(0.6);
  static Color shade = const Color(0xFF2F3541).withOpacity(1);

  // static Color bg = const Color.fromARGB(255, 41, 36, 69).withOpacity(1);

  /////
  // static Color primary = const Color(0xFFBB86FC).withOpacity(0.8);
  // static Color primary = const Color.fromARGB(255, 16, 102, 16);
  // static Color primary = const Color(0xFF075E54);
  static Color primary = const Color(0xFF0A754E);
  static Color primaryBright = const Color.fromARGB(255, 36, 112, 36);
  static Color primary02 = primary.withOpacity(0.2);
  static Color primary05 = primary.withOpacity(0.5);
  static Color primary01 = primary.withOpacity(1);
  static Color secondary = const Color(0xFFFCF686).withOpacity(1);
  static Color secondary1 = const Color(0xFFF2F7A1).withOpacity(1);
  static Color error = const Color(0xFFCF6679);
  static Color white = Colors.white;
  static Color white07 = Colors.white.withOpacity(0.7);
  static Color white05 = Colors.white.withOpacity(0.5);
  static Color white03 = Colors.white.withOpacity(0.3);
  static Color white02 = Colors.white.withOpacity(0.2);
  static Color text = Colors.white;
  static Color grey = Colors.grey;
  static Color black = Colors.black;
  static Color red = Colors.red;
  // static Color red05 = Colors.red.withOpacity(0.5);
  static Color red07 = const Color.fromARGB(255, 244, 100, 92).withOpacity(0.7);

  ///chatgpt bg color ///
  static Color bg = const Color(0xFF1E2834).withOpacity(1);
  static Color container = const Color(0xFF40414F).withOpacity(1);
  static Color whitee = const Color(0xFFE1E1E3).withOpacity(1);

  static Color secondaryGPT = const Color(0xFF4A90E2);
  static Color tertiaryGPT = const Color(0xFFE84A5F);

  static const Color bgG = Color(0xFF1E2834); // Dark blue
  static const Color primaryG = Color.fromARGB(255, 16, 102, 16); // Deep green
  static const Color secondaryG =
      Color.fromARGB(255, 74, 134, 232); // Light blue
  static const Color tertiary = Color.fromARGB(255, 255, 149, 0);
}
